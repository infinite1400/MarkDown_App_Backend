...
export const login = async (req: express.Request, res: express.Response) => {
  try {
    ...
    const existing_user = await findByEmail(email).select(
    ...
    const expectedHash = authentication(
      existing_user.authentication.salt as string,
      password
    );
    if (expectedHash != existing_user.authentication.password) {
      return res
        .status(403)
        .json("Password is Wrong ! Enter Correct password !");
    }

    const salt = random();
    existing_user.authentication.sessionToken = authentication(
      salt,
      existing_user._id.toString()
    );
    await existing_user.save();

    res.cookie("MARKDOWN-AUTH", existing_user.authentication.sessionToken, {
      domain: "localhost",
      path: "/",
    });
